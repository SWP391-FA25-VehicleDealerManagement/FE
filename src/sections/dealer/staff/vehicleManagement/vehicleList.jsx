...
import { SearchOutlined, EyeOutlined, CarOutlined, SwapOutlined } from "@ant-design/icons";
  ...
  const rowSelection = {
    ...
    onChange: (selectedRowKeys) => {
      if (selectedRowKeys.length > 3) {
        toast.warning("Chỉ có thể chọn tối đa 3 xe để so sánh", {
          position: "top-right",
          autoClose: 2000,
        });
        return;
    ...
    getCheckboxProps: (record) => ({
      disabled: selectedVehicles.length >= 3 && !selectedVehicles.includes(record.vehicleId),
    }),
  };
